...
class AppTemplate
{
	...
	function my_callback($source){
		return $this->select($source[1]);
	}
    ...
    function fetch_str($source)
    {
	   ...
	   $replacement = function($matches) { return $this->select($matches[1]);}; 
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert' :
					...
					$replacement = function($matches) { return stripslashes(trim($matches[1],'\''));}; 
					
					$str=preg_replace_callback("/(\'\\$[^,]+)/",$replacement,var_export($t, true));
