...
function sys_user_status($user_id,$is_cache = false,$make_cache=false){
	...
	if($data==false){
		...
		$load_info = $GLOBALS['db']->getRow("SELECT (sum(repay_money)-sum(dlr.self_money)) as load_earnings,sum(dlr.repay_money) AS load_repay_money FROM ".DB_PREFIX."deal_load_repay  WHERE has_repay=1 AND ((user_id = ".$user_id." and t_user_id = 0) or t_user_id = ".$user_id.")");
		//已赚利息// 
		...
		//已赚提前还款违约金
		$data['load_tq_impose'] = $GLOBALS['db']->getOne("SELECT sum(impose_money) FROM ".DB_PREFIX."deal_load_repay use index(idx_1) WHERE has_repay=1 AND status = 0 AND ((user_id = ".$user_id." and t_user_id = 0) or t_user_id = ".$user_id.")");
		//已赚逾期罚息
		$data['load_yq_impose'] = $GLOBALS['db']->getOne("SELECT sum(impose_money) FROM ".DB_PREFIX."deal_load_repay use index(idx_1) WHERE has_repay=1 AND status in (2,3) AND ((user_id = ".$user_id." and t_user_id = 0) or t_user_id = ".$user_id.")");
		...
		$load_wait = $GLOBALS['db']->getRow("SELECT sum(dlr.self_money) as total_self_money,sum(dlr.repay_money) as total_repay_money,sum(dlr.repay_money-dlr.self_money) as load_wait_earnings FROM ".DB_PREFIX."deal_load_repay dlr left join ".DB_PREFIX."deal_load dl on dlr.load_id = dl.id where ((dlr.user_id = ".$user_id." and t_user_id = 0) or t_user_id = ".$user_id.") and has_repay = 0 and (pP2PBillNo ='' or pP2PBillNo is null) ");
...
function FW_DESPWD($pwd){
	...
	$pwd = substr($pwd,strlen(LOGIN_DES_KEY().""),strlen($pwd));
