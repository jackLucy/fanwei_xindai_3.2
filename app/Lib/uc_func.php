...
// +----------------------------------------------------------------------
	...
	function get_user_lock_money_log($limit,$user_id,$type=-1,$condition){
		...
		if($count > 0){
			...
			$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_lock_money_log where user_id = ".$user_id." $extWhere $condition order by create_time id limit ".$limit);
		}
		return array("list"=>$list,'count'=>$count);
	}
	...
	function get_user_point_log($limit,$user_id,$type=-1,$condition){
		...
		$list = array();
		if($count > 0)
			$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_point_log where user_id = ".$user_id." $condition order by create_time id limit ".$limit);
		return array("list"=>$list,'count'=>$count);
	}
